...
# Remplacer "__replace__" par la valeur souhaitée ('__REPLACE__", 'albert einstein', 'louis xiv', 'dupont', ...)
SELECT distinct ?natureLabel ?item ?itemLabel ?articleLabel ?itemDescription
    ...
    WITH {
      SELECT * WHERE {
        BIND (LCASE("léonard de vinci") AS ?searchfor1) 
    ...
    WITH {
      ...
      WHERE {
        INCLUDE %p
#         BIND (CONCAT("haswbstatement:P31=Q5 ", ?searchfor) AS ?searchstr2)      # FULL TEXT
...
#           bd:serviceParam mwapi:gsrsearch ?searchstr2 .
     ...
     LIMIT 1500 # limite pour accélérer la requête
    } AS %i
    WHERE {
      INCLUDE %i
      INCLUDE %p
