...
```sparql
SELECT  ?num  ?item  ?itemLabel ?itemDescription ?article
# SAMPLE et GROUP_CONCAT évite les doublons dans les résultats (images et natures multiples pour 1 item)
...
GROUP BY ?item ?itemLabel ?itemDescription  ?num ?article
ORDER BY ASC(?num)
```
